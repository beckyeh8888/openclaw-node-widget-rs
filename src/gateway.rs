...
fn handle_chat_response(chat_state: &Arc<Mutex<crate::chat::ChatState>>, payload: Option<&Value>) {
    ...
    if let Some(agents) = payload.get("agents").and_then(Value::as_array) {
        ...
        if let Ok(mut cs) = chat_state.lock() {
            // Sync active_session_key with the active agent's session key
            if let Some(active_agent) = agent_list.iter().find(|a| a.id == cs.active_agent_id) {
                cs.active_session_key = active_agent.session_key.clone();
                info!(
                    agent_id = %cs.active_agent_id,
                    session_key = %cs.active_session_key,
                    "synced active session key from agents.list"
                );
            }
